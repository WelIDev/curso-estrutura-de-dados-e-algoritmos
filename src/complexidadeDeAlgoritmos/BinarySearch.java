...
public class BinarySearch {
    ...
    public static int binarySearch(int elem, int[] arr) {
        return binarySearchTailSearch(elem,arr,0,arr.length-1);
    }
    private static int binarySearchTailSearch(int elem, int[] arr, int start, int finish) {
        if (start > finish) {
            return -1;
        }
        ...
        if (elem == arr[middle]) {
            ...
            return binarySearchTailSearch(elem, arr, start, middle - 1);
        }else {
            return binarySearchTailSearch(elem, arr, middle + 1, finish);
        }
    }
}
